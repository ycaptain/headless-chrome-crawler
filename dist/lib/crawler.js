...
const { resolveUrl, debugConsole, debugDialog, tracePublicAPI, } = require('./helper');
const GOTO_OPTIONS = [
    'timeout',
    'waitUntil',
];
...
class Crawler {
     ...
     */
    async _setCookie() {
        if (!this._options.cookies || isEmpty(this._options.cookies))
            return;
    ...
    async _waitFor() {
        if (!this._options.waitFor)
            return;
        await this._page.waitFor(this._options.waitFor.selectorOrFunctionOrTimeout, this._options.waitFor.options, ...(this._options.waitFor.args || []) // eslint-disable-line comma-dangle
        );
    }
    /**
     * @return {!Promise}
